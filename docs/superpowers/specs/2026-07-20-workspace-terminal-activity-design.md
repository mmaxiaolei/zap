...
5. Workspace 标题栏重新 render 时计算当前 workspace 的 running 数量。
6. running 数量大于 0 时渲染 Neon Capsule；否则不渲染。

命令完成后：

1. `TerminalView` 回到 `TerminalViewState::Normal` 或 `Errored`。
2. 同一事件链触发 workspace render。
3. 聚合数量减少；降到 0 时活动状态消失。

## 聚合规则

聚合入口应优先复用 `SessionNavigationData::all_sessions(ctx)` 与 `RunningSessionSummary`。
...
- `WorkspaceTerminalActivitySummary`：负责从当前 app context 汇总 running count。
...
- running 数量超过 99：显示 `99+`。
- terminal 从 long-running 变为 errored：不再计入 workspace activity；tab indicator 仍可展示错误状态。
...
- 聚合 0 个 running sessions 时返回 0。
- 聚合同一 workspace 多窗口 running sessions 时返回正确数量。
- 不计入 read-only session。
- 不计入 shared session viewer。
- `99+` 显示格式正确。
